...
internal class SqlCreateTableWriter : ISqlCreateTableWriter
{
    ...
    public SqlCreateTableWriter(string tableName, ColumnOptions columnOptions, IDataTableCreator dataTableCreator)
    {
    ...
    public string GetSql()
    {
        ...
        sql.AppendLine(Invariant($"EXECUTE IMMEDIATE 'CREATE TABLE {_tableName} ( "));
        ...
        using (var dataTable = _dataTableCreator.CreateDataTable())
        {
            ...
            foreach (DataColumn column in dataTable.Columns)
            {
                ...
                if (common != null && common.NonClusteredIndex && common != _columnOptions.PrimaryKey)
                    ix.AppendLine(Invariant($"CREATE NONCLUSTERED INDEX [IX{indexCount++}_{_tableName}] ON {_tableName} ([{common.ColumnName}]);"));
            }
        }
        ...
        if (_columnOptions.ClusteredColumnstoreIndex)
            sql.AppendLine(Invariant($"CREATE CLUSTERED COLUMNSTORE INDEX [CCI_{_tableName}] ON {_tableName}"));
    ...
    private static string GetColumnDDL(OracleColumn column)
    {
        ...
        sb.Append(column.DataTypeAsString.ToUpperInvariant());

        if (OracleDataTypes.DataLengthRequired.Contains(column.DataType))
            sb.Append('(').Append(column.DataLength == -1 ? OracleDataTypes.MaxSizeMap[column.DataType].ToString() : column.DataLength.ToString(CultureInfo.InvariantCulture)).Append(')');
