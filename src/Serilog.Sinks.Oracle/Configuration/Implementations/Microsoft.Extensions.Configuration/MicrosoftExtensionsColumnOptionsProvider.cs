...
internal class MicrosoftExtensionsColumnOptionsProvider : IMicrosoftExtensionsColumnOptionsProvider
{
    ...
    private static void SetCommonColumnOptions(IConfigurationSection section, OracleColumn target)
    {
    ...
    {
        var newcols =
            config.GetSection("additionalColumns").Get<List<OracleColumn>>()
            ?? config.GetSection("customColumns").Get<List<OracleColumn>>(); // backwards-compatibility
            ...
            foreach (var c in newcols)
            {
                if (!string.IsNullOrWhiteSpace(c.ColumnName))
                {
                    if (columnOptions.AdditionalColumns == null)
                        columnOptions.AdditionalColumns = new Collection<OracleColumn>();
